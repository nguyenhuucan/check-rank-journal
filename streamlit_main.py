    ...
    with tabs[6]:  # Tab Cha: Khác
        ...
        with subtabs[4]:
            if user in unblocked_admins:
                ...
                data = {
                    "STT": [1, 2, 3, 4, 5, 6, 7],
                    "Tổng số tạp chí theo Subject category": [
                        "≥2000", "1500–1999", "1000–1499", "500–999", "200–499", "50–199", "<50"
                    ],
                    "Ngoại hạng chuyên ngành (Q1)": [
                        "<0.5%", "<0.5%", "<0.5%", "<0.5%", "<0.9%", "<2.5%", "<3.5%"
                    ],
                    "Hạng 1 (Q1)": ["<1%", "<2%", "<3%", "<4%", "<5%", "<6%", "<7%"],
                    "Hạng 2 (Q1)": ["<5%", "<6%", "<7%", "<8%", "<10%", "<11%", "<15%"],
                    "Hạng 3 (Q1, Q2)": ["<10%", "<11%", "<12%", "<13%", "<15%", "<16%", "<20%"],
                    "Hạng 4 (Q1, Q2)": ["<18%", "<19%", "<20%", "<21%", "<23%", "<24%", "<28%"],
                    "Hạng 5 (Q1, Q2, Q3)": ["<30%", "<31%", "<32%", "<33%", "<35%", "<36%", "<40%"],
                    "Hạng 6 (Q1, Q2, Q3)": ["<43%", "<44%", "<45%", "<46%", "<48%", "<49%", "<53%"],
                    "Hạng 7 (Q1, Q2, Q3)": ["<56%", "<57%", "<58%", "<59%", "<61%", "<62%", "<66%"],
                    "Hạng 8 (Q1, Q2, Q3)": ["<69%", "<70%", "<71%", "<72%", "<74%", "<75%", "<79%"],
                    "Hạng 9 (Q1, Q2, Q3, Q4)": ["<82%", "<83%", "<84%", "<85%", "<87%", "<88%", "<92%"],
                    "Hạng 10 (Q1, Q2, Q3, Q4)": ["≥82%", "≥83%", "≥84%", "≥85%", "≥87%", "≥88%", "≥92%"]
                }

                df = pd.DataFrame(data)

                st.title("Bảng Xếp Hạng Tạp Chí Theo Subject Category")
